...
fn report(mode: &str, input: usize, archive: usize, roundtrip: u32) {
    ...
    // integer only (operator rule: no float): ten-thousandths
    let ratio_e4: u128 = if archive > 0 {
        (input as u128 * 10_000) / archive as u128
    } else {
        0
    };
    let bpc_e4: u128 = if input > 0 {
        (archive as u128 * 8 * 10_000) / input as u128
    } else {
        0
    };
    println!(
